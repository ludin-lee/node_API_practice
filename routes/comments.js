...
router.post("/comments/:postId", async (req, res) => {
  try{
    ...
    const [existsPost] = await Posts.find({ _id: postId }); //게시글들 id 모으기

    if(user===undefined){ 
      res.status(400).json({"message": "유저네임을 입력해주세요"});
    }else if(password===undefined){
      res.status(400).json({"message": "패스워드를 입력해주세요"});
    }
    else if(content===undefined||content===null){
      res.status(400).json({"message": "댓글 내용을 입력해주세요."});
    }else if(existsPost ===undefined){
      res.status(400).json({"message": "해당 게시글이 없습니다. 다시 한 번 확인해주세요"});
    }else{ 
